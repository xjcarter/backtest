...
class MoBackTest(BackTest):
    def __init__(self, security, json_config, ref_index=None):
        ...
        self.length = self.settings.get('Length', None)
    ...
    def entry_OPEN(self, cur_dt, bar, ref_bar=None):
        ...
        if self.price_series.count() > self.length + 1:
            barX = self.price_series.valueAt(self.length)
            bar0 = self.price_series.valueAt(0)
            rtn = bar0['Close']/barX['Close'] - 1
            if rtn <= self.threshold:
                self.current_trade = self.enter_trade( TradeType.BUY, bar['Date'], self.security, bar['Open'],
                                                                            label=f'RTN{self.length}_{self.threshold}' )
                if self.LONG:
