...
class PctBackTest(BackTest):
    ...
    def entry_OPEN(self, cur_dt, bar, ref_bar=None):
        ...
        if self.price_series.count() > 1:
            bar1 = self.price_series.valueAt(1)
            bar0 = self.price_series.valueAt(0)
            rtn = bar0['Close']/bar1['Close'] - 1
            if rtn <= self.threshold:
                self.current_trade = self.enter_trade( TradeType.BUY, bar['Date'], self.security, bar['Open'], label=f'PCT{self.threshold}' )
                if self.LONG:
