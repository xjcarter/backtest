...
from indicators import StDev, DataSeries, MA
from backtest import BackTest, TradeType, DumpFormat
import calendar_calcs


class TenBackTest(BackTest):
    def __init__(self, security, json_config, ref_index=None):
        ...
        self.MA = MA(self.settings.get('MA', None))
        self.threshold = self.settings.get('Threshold', None)

        self.price_series = DataSeries(derived_len= 20)
    ...
    def entry_OPEN(self, cur_dt, bar, ref_bar=None):
        ...
        if self.MA.count() > 1:
            pt1 = self.MA.valueAt(1)
            pt0 = self.MA.valueAt(0)
            yesterday = self.price_series.valueAt(0)

            r = pt0/pt1 - 1

            if r < self.threshold and yesterday['Close'] < yesterday['Open']:
                self.current_trade = self.enter_trade( TradeType.BUY, bar['Date'], self.security, bar['Open'], label=f'MA{self.threshold}' )
                if self.LONG:
